...
class ReposRequester(BaseRequester):
    ...
    def __get_single_commit(self, username, repo_name, commit_sha: str, commit_list: list, count_list: list):
        try:
            ...
            response = requests.get(commit_url, headers=self._random_header())
            ...
            if issue_url:
                commit_dict = None
                print(issue_url)
                issue_response = requests.get(issue_url, headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                ...
                '''
                f = open('../result/ApacheResult.txt', 'a+')
                f.write(f'{issue_id}, {issue_name}, {issue_url}, {commit_url},{repo_name}\n')
...
if __name__ == '__main__':
    ...
    user_list = [{'login': 'apache', 'id': None, 'email': None, 'location': None, 'hireable': None, 'public_repos': 31, 'followers': 4, 'following': 1, 'repos': ['https://github.com/apache/kafka']}]
